...
1. `gem install bundler`, ignoring any _rvm_ errors (they can occur if both rvm and rbenv are installed)
...
An alternative would have been an in-memory database, however, the data is simple enough that it would add unneeded complexity.
...
1. While this is happening, the job checks if the website URL has already been cached in Redis from an earlier request _and_ if the cached result has an Open Graph image URL. If so, the backend returns the Open Graph URL without trying to fetch it. Otherwise, the backend kicks off a job to fetch the website's Open Graph image URL. It stores the result in a hash, which may contain the Open Graph URL or an error message.
...
[sidekiq](https://github.com/mperham/sidekiq) is [much simpler to setup and use](https://github.com/mperham/sidekiq/wiki/Getting-Started) compared to the alternative resque. This is important with little control over the systems OpenGraphPreviewer is being installed on.
